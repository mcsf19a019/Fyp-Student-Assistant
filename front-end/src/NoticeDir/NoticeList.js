import {Link} from 'react-router-dom';
import { Fragment } from 'react';
const NotieList = ({ notices,user }) => {
    const checkuser = (_id) => {
        if(user == "admin")
        {
            return (
            <Fragment>
            <Link  to={`/delete/${_id}`} className="ss_icons">
            <img  className="ss_img" src="data:image/png;base64," alt="delete icon" width="25" heigth="25" />
            </Link>
            <Link to={`/update/${_id}`} className="ss_icons">
            <img   className="ss_img" src="data:image/png;base64," alt="update icon" width="30" heigth="30" />
            </Link>
            </Fragment>)
        }
        else return null;
    }
    return ( 
        <div className="noticeList">
            {notices.map( notice => (
            <div className="noticePreview" key={notice._id}>
              <Link to={`/notices/${notice._id}/${user}`}><h4>{notice.title.toUpperCase()}</h4>
              <p><b>Publish Date:</b> {notice.date}</p>
              </Link>
                <div className="check">
                    {checkuser(notice._id)}
                </div>
            </div>
        )
        )}
        </div>
     );
}
 
export default NotieList;